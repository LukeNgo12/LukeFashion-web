...
<template>
  <div>
    <div class="cursor-pointer flex font-semibold pt-8 pb-4 leading-none justify-between items-center text-gray-900 dark:text-white" @click="isOpen = !isOpen">
      <span>{{$t('filtering.starRatingFilter.rating')}}</span>
      <ion-icon v-show="isOpen" name="ion:chevron-up-outline" class="dark:text-gray-400" />
      <ion-icon v-show="!isOpen" name="ion:chevron-down-outline" class="dark:text-gray-400" />
    </div>
    <div v-if="isOpen" class="mt-3 text-sm grid text-gray-500 dark:text-gray-300 gap-3">
      ...
      <div class="cursor-pointer flex gap-2 items-center">
        ...
        <label class="flex items-center" for="star-four">
          ...
          <span class="ml-1 text-xs">{{$t('filtering.starRatingFilter.up')}}</span>
        </label>
      </div>
      <div class="cursor-pointer flex gap-2 items-center">
