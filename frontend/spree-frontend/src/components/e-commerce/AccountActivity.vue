...
<template>
  <div>
    <!-- Page Header -->
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">{{$t('components.accountActivity.accountActivity')}}</h1>
      <p class="text-gray-600 dark:text-gray-400">{{$t('components.accountActivity.trackAccountAcitvity')}}</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xs border border-gray-100 dark:border-gray-700">
      <div class="p-6 md:px-8 pb-4 border-b border-gray-100 dark:border-gray-700">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{$t('components.accountActivity.recentActivity')}}</h3>
      </div>
          ...
          <div v-for="(activity, index) in filteredActivities" :key="activity.id" class="flex gap-4 relative">
            ...
            <div class="shrink-0">
              <div class="w-10 h-10 rounded-full flex items-center justify-center" :class="activity.iconBg">
                <ion-icon :name="activity.icon" size="20" :class="activity.iconColor" />
              </div>
            </div>
            ...
            <div class="flex-1 min-w-0 pt-0.5">
              <div class="flex items-start justify-between gap-4 mb-1">
                <h4 class="font-semibold text-gray-900 dark:text-white">{{$t('components.accountActivity.title')}}</h4>
                <time class="text-sm text-gray-500 dark:text-gray-400 shrink-0">{{ getRelativeTime(activity.timestamp) }}</time>
              </div>
              <p class="text-sm text-gray-600 dark:text-gray-400">{{$t('components.accountActivity.description')}}</p>
            </div>
          </div>
        </div>

        <div v-else class="text-center py-12 text-gray-500 dark:text-gray-400">{{$t('components.accountActivity.noActivityFound')}}</div>
      </div>
      ...
      <div class="p-6 pt-4 bg-gray-50 dark:bg-gray-800 rounded-b-lg border-t border-gray-100 dark:border-gray-700">
        <button class="text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 flex items-center gap-2 transition-colors">
          <ion-icon name="ion:download-outline" size="18" />
          <span>{{$t('components.accountActivity.exportActivityLog')}}</span>
        </button>
      </div>
    </div>
  </div>
